...
int switchState = 0;
const int CENTRAL_duty = 85;

const int fsm_interval = 500; // FSM interval in milliseconds
...
// Set possible states
enum {STATE_WAIT, STATE_START_LIFT, STATE_POS_TORQUE, STATE_NEG_TORQUE, STATE_STOP_LIFT, STATE_START_FORWARD,
  STATE_LEFT25, STATE_RIGHT25, STATE_REAR25, STATE_ALL25, STATE_LEFT0, STATE_RIGHT0, STATE_REAR0, STATE_START};
unsigned char state = STATE_WAIT; // Initialize state to WAIT
unsigned char nextState = STATE_POS_TORQUE;
...
void fsm_step() {
  switch (state) {
    ...
    case STATE_POS_TORQUE:
      Serial.printf("current state: STATE_POS_TORQUE\n");
    ...
    case STATE_NEG_TORQUE:
      Serial.printf("current state: STATE_NEG_TORQUE\n");
      set_hovercraft_forces(0,0,-1.0);
      delay(1000);
      state = STATE_STOP_LIFT;
      break;
    ...
    case STATE_START_FORWARD:
      ...
      // TESTING:
      Serial.printf("Successfully ran all cases in test\n");
      set_hovercraft_forces(0,0,0);
      fan.write(0);
      state = STATE_WAIT;
      break;
